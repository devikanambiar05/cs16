...
| **FAQsPage layout overhaul** | Removed "Knowledge Base" heading and subheading; category pills now smaller (`px-2 py-1 text-xs`) inline with the search row above main content; search bar is inside the Topics sidebar (not in the main header row); no hero section |
